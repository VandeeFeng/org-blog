#+title: Readwise 和 GitHub Actions 联用 - 流动知识检索
...
主要思路是，在 push 的时候会运行这个 Python 脚本，读取第一行内容，获取 URL 使用 readwise 的 API 保存。
** python
...
def trigger_workflow():
    ...
    try:
        ...
        logging.info("Successfully triggered the other workflow")
...
def main():
    try:
        # 读取第一行
        with open('bookmarks.md', 'r', encoding='utf-8') as f:
            first_line = f.readline().strip()

        # 检查是否包含 #savemd 标签
        if has_save_tag(first_line):
            logging.info("Found #savemd tag, triggering other workflow")
            trigger_workflow()

        # 提取URL并发送到Readwise
        url = extract_url(first_line)
        if url:
            ...
            logging.error("No URL found in the first line")
