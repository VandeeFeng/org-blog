...
- Org-remark lets you highlight and annotate text files, websites, EPUB books and Info documentation with using Org mode.
  github: https://github.com/nobiot/org-remark
  via: https://www.chiply.dev/post-annotate-in-place

  我还是觉得要保证笔记格式的通用性，尽量少使用扩展的专用格式。之前在 Logseq 里就吃了这个亏，在转移笔记的时候要做大量的格式调整。我也尽量不把笔记限定在 org 或 md 里。
...
和 AI 相关的整理到了 [[https://www.vandee.art/blog/2025-08-02-pkm-manual-combined-ai.html][PKM（个人知识管理）构建手册 - with AI]] 。

AI 的迭代发展用星期来计算我觉得一点也不夸张。从 function tool 到 agent framework，再到 Claude Code、agent harness，对于 PKM 和做笔记，流程上的便利其实不那么关键。一句 prompt 可以找到需要了解的全部资料，模型可以马上给出的所有相关回答，但我不喜欢省略“求索”的过程。很多时候，最有价值的部分恰恰是问题的发现、答案的寻找。

所以我现在的 PKM 和 AI 的结合一直很浅。有了 AI 之后也没有觉得有太多地方必须需要它的参与，最有帮助的还是启发式的讨论，帮我提出更多的问题。

*** Intro

关于 RAG 和各类知识文档管理聚合项目，我目前的想法是，如果没有成千上万的笔记，或者专业学习和科研，普通的 tag + 关键字检索 足够用了，有了适合自己的笔记管理方法，AI 的检索其实不那么必须。再一个，我个人还是习惯自己完成最原始的笔记，AI 帮助我发散思考，而不是生成笔记。

现在能不用 AI 搜索就不用了，不想把这些没什么质变的便利建立在 AI 对互联网内容的侵略上。

但是 AI 搜索对于扩展 agent 的能力来说是很必要的。和 robots.txt 这个君子协定类似，我觉得规范是规范，主要还是看个人。对于内容创作者来说，AI 检索到自己手写的内容，也是有利有弊。很可能今后互联网的部分内容就是 AI 生成的了，正是因为如此，所以我才更原意坚持手写。写给自己的东西，考虑那么多干嘛。

最大的问题可能还是在知识版权这一块，但是现在对于 LLM 的生成内容还没有严格的界定。如果创作者的目的是内容能够被看到，那么传统搜索引擎和 AI 搜索的区别也不是很大。

但是还是纪念一下 perplexity 之前搜索到了自己博客的内容🤣
...
很早之前就陆续看到过和这篇文章相关的内容，开始也不是很有所谓，毕竟 local LLM 开始的时候 Ollama 就是很方便。随着对 LLM 的学习，配置对我来说已经不是很麻烦了，local LLM 领域现在也百花齐放。我已经停止使用 Ollama，转为使用 llama.cpp 了。llama.cpp 现在也直接提供 chat UI。

[[https://github.com/AlexsJones/llmfit][AlexsJones/llmfit]] 这个项目能快速选择适合自己硬件的本地模型，挺实用的。

我觉得对于普通用户来说，用付费 API（不是订阅）强模型来做一些复杂的推理生成计划，本地模型来执行是性价比很高的方案。Mac mini + 本地模型，效果真挺不错的。

*** LLM in Emacs

[[https://github.com/xenodium/agent-shell][xenodium/agent-shell]] 用过一段时间，在 Emacs 里挺好用。在 Emacs 里，主要是为了把代码定位之后发送给 agent。

[[https://github.com/milanglacier/minuet-ai.el][milanglacier/minuet-ai.el]] 再就是这个自动补全。

其他时间基本都在终端里了。

**** Aider

[[https://github.com/Aider-AI/aider][Aider]] 最近挺火的一个 CLI AI 助手，GitHub 20.3K Star. 在 Emacs 有 [[https://github.com/tninja/aider.el][aider.el]] 、[[https://github.com/MatthewZMD/aidermacs][aidermacs]] 。

可以像 cursor 一样有 diff，修改代码，比 [[https://github.com/ErikBjare/gptme][gtpme]]、[[https://github.com/BuilderIO/ai-shell][ai-shell]] 、[[https://github.com/cline/cline][cline]] 、[[https://github.com/Doriandarko/claude-engineer][claude-enginner]] 全面许多。
...
[[https://github.com/karthink/gptel][gptel: A simple LLM client for Emacs]] 是 Emacs 的一个包，可以在 Emacs 里直接和 LLM 对话，目前在用。
...
- 2026-06-21: 更新 PKM with LLM 部分，删除了我已经没有使用的。
- 2026-07-06：再编辑 PKM with LLM 部分。
