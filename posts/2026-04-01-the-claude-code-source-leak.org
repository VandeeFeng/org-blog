...
今年热度比较高的应该是聚焦安全的 agent sandbox 以及聚焦效率编排的 parallel agent。就我了解到的，parallel agent 相关的项目里，interface 还是 TUI 和 CLI 的居多。LLM 天然对 shell command 和文本的适应性好，交互成本也更低，CLI 作为和 LLM 交互的接口其实是很自然而然的一个结果。MCP 到现在其实也从侧面验证了 CLI 是更合适的，例如最近 GitHub 上比较热门的 [[https://github.com/jackwener/opencli/][opencli]] 。
...
CLIs are composable because they came originally from Unix and that is the [[https://tldp.org/LDP/GNU-Linux-Tools-Summary/html/c1089.htm][Unix tools philosophy]]: tools were designed so that they could operate together.
...
[[https://interconnected.org/home/2026/05/20/resident][Resident: vibe coding firmware (our new sandbox library for ESP32 devices) (Interconnected)]] 这篇文章里，提到：It’s not an outrageous extrapolation! [[https://taalas.com/products/][Taalas]] is baking LLMs into silicon and delivers 17k tokens per second per user on Llama 3.1 8B ([[https://chatjimmy.ai/][try it here]], e.g. 4,000 words on Hamlet as a space opera, it’s wild it’s so instant). So GPT-4-equiv is a matter of time.
