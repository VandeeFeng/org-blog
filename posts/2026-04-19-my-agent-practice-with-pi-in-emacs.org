    ...
    async function askPermission(toolName: string, input: string, ctx: any): Promise<boolean> {
        ...
        const choice = await ctx.ui.custom<boolean>((tui, theme, _kb, done) => {
            ...
            return {
                render: () => {
                    const content = [
                        theme.bold(theme.fg("warning", `⚠️ ${toolName}: ${input}`)),
                        `${renderOption("Yes", selected === 0)}  ${renderOption("No", selected === 1)}`,
                        theme.fg("dim", "←→ / tab navigate  enter select  esc cancel"),
                    ];
                    const innerWidth = content.reduce((max, line) => Math.max(max, visibleWidth(line)), 0);
