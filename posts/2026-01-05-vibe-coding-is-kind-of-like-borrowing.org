...
它们就像贷款，让 AI 帮自己做的越多，将来要还的就越多。(代码水平不高使用者的 vibe coding)
...
Naur theory 诺尔理论：
诺尔理论指的是程序员对代码行为的心理直觉或心智模型。在 Peter Naur 1985年的经典论文《Programming as Theory Building》中，他提出了一个革命性的观点：
...
你的AI编程助手，也就是你用来写代码的那个，需要降低你的维护成本。而且不是降低一点点。你现在写代码的速度快了两倍？最好希望你的维护成本也减半了。生产力提高了三倍？维护成本就要降到三分之一。否则，你就完蛋了。你是在用暂时的速度提升换取永久的束缚。[...]

只有当 LLM 降低你的维护成本，且降低幅度恰好与它增加代码的速度成反比时，这个算式才成立。如果你的产出翻倍，维护这些产出的成本也翻倍，那么二乘以二意味着你的维护成本翻了四倍。如果你的产出翻倍，但维护成本保持不变，二乘以一意味着你的维护成本仍然翻了一倍。

— [[https://www.jamesshore.com/v2/blog/2026/you-need-ai-that-reduces-your-maintenance-costs][James Shore]], You Need AI That Reduces Maintenance Costs

via: https://simonwillison.net/2026/May/11/james-shore/

2026-08-04 added:

The joy of working on personal projects comes from [[https://ankursethi.com/blog/results-are-a-side-effect-of-doing-the-work-with-love/][the process, not from the outcome]]. [...]
...
via: https://ankursethi.com/blog/prevent-cognitive-debt-by-manually-retyping-llm-generated-code/ , https://news.ycombinator.com/item?id=49153374

作者的这些想法也是我一直对 LLM 生成代码的倾向。那些已经很熟悉的“重复无聊”的代码构建流程交给 LLM 是很省事的，特别是根据现有代码做类推的时候。
...
这篇 blog 讨论的范围很窄，就是 vibe coding，不是 AI 和 agent。

计算器可以帮我们完成日常生活中的计算需求，但这不意味着数学不需要学习，vibe coding 和编程的关系也是。

对于 AI 代码生成，我不喜欢的地方在于：它把我觉得编程这个爱好里最有趣的地方替代了。对于工程实现，我完全接受 AI 代码的 “工业化” 方向。多年之后看现在手写编码的感觉可能就像现在回到没有电力、没有网络、没有智能电子设备的时候一样。

但当 AI 软件开发的流程达到现在的工业水平之后，手写代码仍然可以是爱好。徕卡也出了数码机，甚至还有只能拍黑白的，徕卡独特的色彩科学、镜头和机内模拟，是徕卡这个品牌价值一致延续的内核。

无论是 AI 代码还是手写代码，品味以及哪些独特的味道是可以延续的。而且比起用自然语言 prompt 反复和 AI 沟通细节，我还是更喜欢更直接的思维到代码的逻辑映射，编码逻辑 -> 自然语言 -> LLM 黑箱 -> 代码实现，这个 LLM 编码的路径里，编码逻辑到自然语言是有损耗的，后面的每一步也都是有损耗的，手写代码这方面就是更直接：编码逻辑 -> 编程语言 -> 代码实现。就像书法绘画、吉他演奏、作曲歌唱一样，王羲之的兰亭序只有在这种心流体验下，才有可能偶然天成。绘画艺术作品里，那些笔触都是当下和外界反馈的最直接映射，中间多转换一次是多余的。

我一直认为自然语言作为代码的中间载体不合适。自然语言语义的模糊性是它的功能决定的，人类需要用语言来沟通，表达，交流。对于机器和代码，数学和逻辑才是最合适的中间载体。我用 agent 写代码的许多 prompt 都会直接使用逻辑语言、伪代码，而不是大白话，把 LLM/agent 看作是一个自然语言解释器是更合适的。

比如开发一个天气应用，表达节气的时候，我觉得用 prompt 解释起来实在是太麻烦，代码逻辑直接的多，也避免了 AI 奇奇怪怪的不准确语义的命名：
...
我之前只会基础的 lisp、python 和 C，在写 rust 代码的时候，这样就相当于是代码转写，生成的代码质量会高很多，也省去了许多看文档初次学习的过程，看多了 rust，自然也就学会了。

AI 代码生成之后，代码更像是是机器执行结果的中间过程，显得不那么重要了，也确实没有必要把代码书写看的太重。但编程就像写作，作为一个爱好，我还是更喜欢这个过程给我带来的乐趣，这是编程艺术体现的核心。
