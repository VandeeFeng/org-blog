...
在 Emacs 的快捷键设置里 Ctrl 记作 C，Alt 和 Option 记作 M，Shift 记作 S，Win 和 Command 记作 s(super 键)，组合键用 ~-~ 链接起来，ctrl+c 记作 ~C-c~ 。Emacs 可以单独自定义 major mode，用 hook 在不同的编程语言里激活不同的自定义 major mode，相同的快捷键就可以实现不同的功能了。
