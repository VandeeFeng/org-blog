...
#+description: 现在 vibe 这么方便了，为什么还要在这个上面花这么多心思？我觉得用键盘打字 coding 和弹吉他的感觉很像。在 Emacs 里配置这些键位对我来说，就像在调琴弦，调合成器一样。vibe 更多的是享受结果，而这些则是享受过程。
#+filetags: Emacs
...
在 Emacs 的快捷键设置里 Ctrl 记作 C，Alt 和 Option 记作 M，Shift 记作 S，Win 和 Command 记作 s(super 键)，组合键用 ~-~ 链接起来，ctrl+c 记作 ~C-c~ 。

现在配置了 m 作为 mark 的 leader key。Emacs 里的 mark 相当于 vim 里的 visual 模式。但是 Emacs 默认的 mark 键位我很不习惯，vim 里 visual 模式快速选择 symbol，word，sentence 中英混合再加上代码又很不方便。把 m 绑定在了 Emacs 的 ~mark-sexp~ 上，现在 mm 就可以连续 mark 了。

vim 里的 f，w，e，b，再加上 Emacs 里自带的 ~forward-sexp~ , ~backward-sexp~ ,在代码行内的移动就很方便了。
...
在 vim 里，快速删除前后的内容可以使用 d 加上方向导航键位，例如 dl,dw,db,de，也可以使用 c 在删除之后直接进入 insert。但在一些 insert 模式场景的编辑不那么便利。

最恶心的就是在输出各种成对出现的 ~()~ , ~{}~ , ~""~ 的时候，这偏偏又是代码里频率很高的。输入完括号内的内容之后，移动光标到括号外再输入，这个过程在 vim 里要先退出 insert，移动光标，再进入 insert，而组合键只需要一次移动光标的操作（Emacs 里是 Ctrl+f,Ctrl+b），这种场景下我觉得组合键的优势是很明显的，也更符合直觉。当然习惯了也都差不多，多一次 jk 也不麻烦，但有时候就是觉得有点膈应。

Vim 最核心设计的就是它的三个编辑模式，我觉得便利的移动导航是 vim 的灵魂，弊端就是需要频繁的切换模式。Normal 模式 c，a，i，d，x，s 提供了非常全面、便捷的增添删减方法，但是 insert 模式下就只是 insert（可以临时执行一次 normal 指令）。因此在 Emacs 里补足 insert 模式就好了。
...
** mark & move
包含了设定 mark 方便在代码编辑里来回跳转，defun，symbol，word 的快速复制剪切。
