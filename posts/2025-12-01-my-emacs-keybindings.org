...
Vim 最核心设计的就是它的三个编辑模式，便利的移动导航是 vim 的灵魂，弊端就是需要频繁的切换模式。Normal 模式 c，a，i，d，x，s 提供了非常全面、便捷的增添删减方法，但是 insert 模式下就只是 insert（可以临时执行一次 normal 指令）。因此在 Emacs 里补足 insert 模式就好了。
...
Vim 里的 f，w，e，b 小范围移动，vi，va 选定函数内部特定的内容，gd 跳转到定义，再加上自定义的 m 序列按键，Emacs 里自带的 ~forward-sexp~ , ~backward-sexp~ 等函数，日常的代码的选中移动基本可以全键盘覆盖。
