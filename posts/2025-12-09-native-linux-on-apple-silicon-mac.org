...
在 x86 的老电脑上体验测试了一段时间，感觉不错，然后就是这台 Mac。当个人电脑 PC 对我来说不再只是一个工具，而是一个伙伴的时候，Linux 就是现在最适合我的操作系统，[[https://www.vandee.art/blog/2025-07-16-linux-is-awesome.html][Linux 太自由了]] 。
...
系统更新了几次之后，现在 100% 差不多是正常可接受的音量了。
...
*** My Sway + i3status config:

这两个配置加起来就是之前界面截图的样子了
...
# Custom
#
    # stacking or floating windows
    # for_window [app_id="librewolf"] floating enable
    for_window [app_id="thunar"] floating enable
    for_window [app_id="opensnitch"] floating enable

    # bindsym $mod+period exec ghostty
    bindsym $mod+Shift+b exec librewolf --kiosk --new-windows
    ...
    # Clipboard
    exec wl-paste --watch cliphist store
    bindsym $mod+v exec --no-startup-id bash -c 'cliphist list | fuzzel --dmenu -p "📋:" | cliphist decode | wl-copy'
...
#
    ...
    # Set default volume (105%)
    exec --no-startup-id pactl set-sink-volume \@DEFAULT_SINK@ 105%
    # Set default brightness (18%)
    exec --no-startup-id brightnessctl set 19%
...
#
    ...
    bindsym $mod+backslash layout stacking
